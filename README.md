...
│   └── pom.xml
├── pacts/                      # Local/shared Pact contracts for manual provider verification
...
Six Pact interactions cover the `order-service` API:
...
| `POST` | `/orders`               | Missing required field — 400      |
...
- Pact contracts are passed through CI artifacts to keep the pipeline reproducible without requiring a Broker.
- JaCoCo gates are intentionally strict to demonstrate automated quality enforcement in CI.
- Pact Broker is optional for local exploration; CI verifies the provider against contracts generated by the consumer job.

---

## Limitations

- **In-memory data only.** `OrderController` holds a static list; there is no database or repository layer. Provider `@State` methods are no-ops because the data is always present. Moving to a real persistence layer would require each `@State` to set up and tear down fixtures explicitly.
- **No Pact Broker integration.** Contracts flow through CI artifacts, so there is no versioning, tagging, `can-i-deploy` gate, or consumer/provider compatibility matrix. The `docker-compose.yml` Broker exists for local experimentation only.
- **No security contracts.** None of the interactions include authentication headers (Bearer tokens, API keys). Adding them would require `matchHeader` entries in every consumer pact and corresponding security configuration on the provider.
- **Single consumer–provider pair.** The demo covers one directed relationship (`user-service → order-service`). Fan-out topologies, diamond dependencies, or bi-directional contracts are not demonstrated.
- **100% JaCoCo gate is intentional but atypical.** The strict threshold is there to demonstrate automated quality enforcement in CI. Most real projects use lower thresholds or exclude generated code explicitly.
- **No contract versioning story.** There is no tagging, semantic versioning of contracts, or backward-compatibility verification between contract revisions.
